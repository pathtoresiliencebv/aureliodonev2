...
export function MediaGrid({ teamSlug, searchParams }: MediaGridProps) {
  ...
  const handleSelectAll = () => {
    if (selectedFiles.length === mockMediaFiles.length) {
      setSelectedFiles([]);
    } else {
        ...
        {mockMediaFiles.map((file) => (
          <Card
            ...
            className={`group cursor-pointer transition-all hover:shadow-md ${
              selectedFiles.includes(file.key) ? 'ring-2 ring-primary' : ''
            }`}
          ...
          >
            <CardContent className="p-2">
              ...
              <div className="space-y-1">
                <p className="text-xs font-medium truncate" title={file.metadata.originalName}>
